...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		...
		<?php
			wp_link_pages( array(
				'before' => '<div class="page-links">' . __( 'Pages:', 'planar' ),
				'after'  => '</div>',
			) );
		?>
	</div><!-- .entry-content -->
...
<?php if ( true === get_theme_mod( 'top_single_display' ) ) { ?>
...
$avatar = get_theme_mod( 'avatar_upload' );
	...
	<?php } ?>
		<div class="author-meta">
			<?php _e( 'Published by ', 'planar'); ?><?php the_author_posts_link(); ?>
			<?php _e( 'on ', 'planar'); ?><time datetime="<?php the_time('Y-m-d'); ?>"><?php the_time('M d, Y'); ?></time>
		</div>
		</div><!-- #author -->
...
<?php } // ! is_active_sidebar( 'sidebar-blog' ) ?>
		...
		<?php
			$category_list = get_the_category_list( __( ', ', 'planar' ) );
			$tag_list = get_the_tag_list( '#', __( ' #', '', 'planar' ) );

			if ( ! planar_categorized_blog() ) {
				if ( '' != $tag_list ) {
					$meta_text = __( '%2$s.', 'planar' );
				} else {
					$meta_text = '';
				}
				...
				if ( '' != $tag_list ) {
					$meta_text = '<p>' . __( 'in %1$s.', 'planar' ) . '<br />tags<br />' . __( '%2$s.', 'planar' ) . '</p>';
				} else {
					$meta_text = '<p>' . __( 'in %1$s', 'planar' ) . '</p>';
				}
