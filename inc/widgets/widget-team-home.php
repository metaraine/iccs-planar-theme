<?php
/**
 * Custom Widget Section of Team for Home Page
 * @package Planar
 */
add_action('widgets_init', create_function('', 'register_widget("Planar_Team_HomeWidgets");'));

class Planar_Team_HomeWidgets extends WP_Widget {
	function __construct() {
		parent::__construct(
			'team_home_widgets',
			'DT ' . __( 'Team  Home Widgets', 'planar' ),
			array(
				'classname' => 'team_home_widgets', 
				'description' => __( 'Team section on the Home Page.', 'planar' ),
			)
		);
 	}

 	function form( $instance ) {
		$planar_defaults[ 'title_section' ] = __( 'Our Team', 'planar' );
 		$planar_defaults[ 'text_main' ] = __( 'Widget display section team (Woo Team) on the Home Page.', 'planar' );
		...
		<p><?php _e( 'Widget display section team (Woo Team) on the Home Page', 'planar' ); ?></p>
	...
	function widget( $args, $instance ) {
		...
		<?php echo do_shortcode('[woothemes_our_team size="200" display_additional="false" limit="'. $limit_num .'" per_row="'. $cols_num .'"]'); ?>
