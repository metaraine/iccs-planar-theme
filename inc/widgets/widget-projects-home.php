<?php
/**
 * Custom Widget Section of Projects for Home Page
 * @package Planar
 */
add_action('widgets_init', create_function('', 'register_widget("Planar_Project_HomeWidgets");'));

class Planar_Project_HomeWidgets extends WP_Widget {
	function __construct() {
		parent::__construct(
			'project_home_widgets',
			'DT ' . __( 'Projects  Home Widgets', 'planar' ),
			array(
				'classname' => 'project_home_widgets', 
				'description' => __( 'Projects section on the Home Page.', 'planar' ),
			)
		);
 	}

 	function form( $instance ) {
		$planar_defaults[ 'title_section' ] = __( 'Portfolio', 'planar' );
 		$planar_defaults[ 'text_main' ] = __( 'Widget display projects (Woo Projects) section on the Home Page.', 'planar' );
		...
		<p><?php _e( 'The widget for displaying section projects (Woo Projects) of the template Home Widgets.', 'planar' ); ?></p>
		...
		<p>
		<label for="<?php echo $this->get_field_id('limit_num'); ?>"><?php _e( 'Number of projects to show:', 'planar' ); ?></label>
	...
	function widget( $args, $instance ) {
		...
		<?php echo do_shortcode('[projects limit="'. $limit_num .'" columns="'. $cols_num .'"]'); ?>
