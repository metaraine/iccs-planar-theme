<?php
/**
 * Custom Widget Section of Testimonials for Home Page
 * @package Planar
 */
add_action('widgets_init', create_function('', 'register_widget("Planar_Testimonial_HomeWidgets");'));

class Planar_Testimonial_HomeWidgets extends WP_Widget {
	function __construct() {
		parent::__construct(
			'testimonial_home_widgets',
			'DT ' . __( 'Testimonials  Home Widgets', 'planar' ),
			array(
				'classname' => 'testimonial_home_widgets', 
				'description' => __( 'Testimonials section on the Home Page.', 'planar' ),
			)
		);
 	}

 	function form( $instance ) {
		$planar_defaults[ 'title_section' ] = __( 'Testimonials', 'planar' );
 		$planar_defaults[ 'text_main' ] = __( 'Widget display section testimonials (Woo Testimonials) on the Home Page.', 'planar' );
 		$planar_defaults[ 'button_text' ] = '';
		$planar_defaults[ 'limit_num' ] = '2';
		$planar_defaults[ 'cols_num' ] = '2';
		...
		<p><?php _e( 'Widget display section testimonials (Woo Testimonials) on the Home Page.', 'planar' ); ?></p>

		<p>
		<label for="<?php echo $this->get_field_id('title_section'); ?>"><?php _e( 'Title:','planar' ); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('text_main'); ?>"><?php _e( 'Intro Text:','planar' ); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('limit_num'); ?>"><?php _e( 'Number to show:', 'planar' ); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('cols_num'); ?>"><?php _e( 'Number of columns:', 'planar' ); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('button_text'); ?>"><?php _e( 'Button Text:', 'planar' ); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('button_url'); ?>"><?php _e( 'Button Link:', 'planar' ); ?></label>
	...
	function widget( $args, $instance ) {
		...
		<?php echo do_shortcode('[woothemes_testimonials limit="'. $limit_num .'" per_row="'. $cols_num .'"]'); ?>
