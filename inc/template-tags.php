...
function planar_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'planar' ); ?></h1>
	...
	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
		...
		<div class="nav-previous"><?php next_posts_link( __( '<i class="fa fa-arrow-left"></i>', 'planar' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( '<i class="fa fa-arrow-right"></i>', 'planar' ) ); ?></div>
		<?php endif; ?>
...
function planar_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'planar' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'planar' ), '<span class="edit-link">', '<span>' ); ?></p>
	<?php
			break;
		default :
	?>
			...
			<div class="comment-box">
				...
				<div class="comment-author-vcard">
					...
					<?php if ( user_can( $comment->user_id, 'manage_options' ) ) : ?>
						...
						<span><?php _e( 'Author', 'planar' ); ?></span>
				...
				<div class="comment-content">
					...
					<?php if ( $comment->comment_approved == '0' ) : ?>
						<em><?php _e( 'Your comment is awaiting moderation.', 'planar' ); ?></em>
						<br />
					<?php endif; ?>

					<?php comment_text(); ?>
<time datetime="<?php comment_time( 'c' ); ?>">
	<?php printf( _x( '%1$s &middot; %2$s', '1: date, 2: time', 'planar' ), get_comment_date(), get_comment_time() ); ?>
				</time>
				</div><!-- .comment-content -->
				...
				<div class="comment-meta">
					...
					?>
					<?php edit_comment_link( __( 'Edit', 'planar' ), '<span class="edit-link">', '<span>' ); ?>
				</div><!-- .comment-meta -->
...
function planar_posted_on() {
	printf( __( '<span class="byline">by <span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span></span> on <time class="entry-date" datetime="%4$s">%5$s</time>', 'planar' ),
		esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
		esc_attr( sprintf( __( 'All posts by %s', 'planar' ), get_the_author() ) ),
		get_the_author(),
...
if ( ! function_exists( 'planar_txt_credits' ) ) {
	function planar_txt_credits() {
		$text = sprintf( __( 'Powered by %s', 'planar' ), '<a href="http://wordpress.org/">WordPress</a>' );
		$text .= '<span class="sep"> &middot; </span>';
		$text .= sprintf( __( 'Theme by %s', 'planar' ), '<a href="http://dinevthemes.com/">DinevThemes</a>' );
...
if ( ! function_exists( 'planar_arhives_title' ) ) {
	function planar_arhives_title() {
		if ( is_category() ) :
			printf( __( 'Category %s', 'planar' ), '<span>' . single_cat_title( '', false ) . '</span>' );
		elseif ( is_tag() ) :
			printf( __( 'Tag %s', 'planar' ), '<span>' . single_tag_title( '', false ) . '</span>' );
		elseif ( is_author() ) :
			printf( __( 'Author %s', 'planar' ), get_the_author() );
		elseif ( is_day() ) :
			printf( __( 'Day %s', 'planar' ), '<span>' . get_the_date() . '</span>' );
		elseif ( is_month() ) :
			printf( __( 'Month %s', 'planar' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );
		elseif ( is_year() ) :
			printf( __( 'Year %s', 'planar' ), '<span>' . get_the_date( 'Y' ) . '</span>' );
		else :
			_e( 'Archive', 'planar' );
		endif;
	}
...
/**
 * Display data custom meta_box Around Title
 */
add_action( 'before_page_title', 'meta_before_title' );
add_action( 'after_page_title', 'meta_after_title' );

function meta_before_title() {
    global $post;

    $title_icon = get_post_meta( $post->ID, 'meta-image', true );

	if( !empty( $title_icon ) ) {
     		echo '<img src="'.$title_icon.'" class="title-img" />';
	}
}
function meta_after_title() {
    global $post;

    $btn_txt = get_post_meta( $post->ID, 'meta-text', true );
    $btn_link = get_post_meta( $post->ID, 'meta-url', true );

	if( !empty( $btn_link ) && !empty( $btn_link ) ) {
     		echo '<a class="btn title-btn" href="'.$btn_link.'">' . $btn_txt . '</a>';
	}
}
